...
var Intercooler = __webpack_require__(4);
...
var Intercooler = Intercooler || function () {
  ...
  if ((typeof Zepto === 'undefined' ? 'undefined' : _typeof(Zepto)) && $ == null) {
    $ = Zepto;
  }
  ...
  var _MACROS = $.map(['ic-get-from', 'ic-post-to', 'ic-put-to', 'ic-patch-to', 'ic-delete-from', 'ic-style-src', 'ic-attr-src', 'ic-prepend-from', 'ic-append-from', 'ic-action'], function (elt) {
  ...
  var _isDependentFunction = function _isDependentFunction(src, dest) {
    if (!src || !dest) {
      return false;
    }
  ...
  function handleRemoteRequest(elt, type, url, data, _success) {
      ...
      success: function success(data, textStatus, xhr) {
        ...
        try {
          if (processHeaders(elt, xhr)) {
            ...
            _success(data, textStatus, elt, xhr);
  ...
  function preventDefault(elt, evt) {
    return elt.is('form') || elt.is('input[type="submit"], button') && elt.closest('form').length == 1 || elt.is('a') && elt.is('[href]') && elt.attr('href').indexOf('#') != 0;
  }
  ...
  function maybeScrollToTarget(elt, target) {
    if (closestAttrValue(elt, 'ic-scroll-to-target') != "false" && (closestAttrValue(elt, 'ic-scroll-to-target') == 'true' || closestAttrValue(target, 'ic-scroll-to-target') == 'true')) {
  ...
  function fireICRequest(elt, alternateHandler) {
    ...
    if (elt.length > 0) {
      ...
      var invokeRequest = function invokeRequest() {
  ...
  function newIntercoolerHistory(storage, history, slotLimit, historyVersion) {
    ...
    function historyConfigHasChanged(historySupportData) {
      return historySupportData == null || historySupportData.slotLimit != slotLimit || historySupportData.historyVersion != historyVersion || historySupportData.lruList == null;
    }
    ...
    function saveHistoryData(restorationData) {
      ...
      try {
        ...
        try {
          ...
          log(getTargetForHistory($('body')), "Unable to save intercooler history with entire history cleared, is something else eating " + "local storage? History Limit:" + slotLimit, "ERROR");
        }
      }
    }
  ...
  if ($.ajaxTransport) {
    $.ajaxTransport("text", function (options, origOptions) {
      if (origOptions.url[0] == "#") {
        ...
        return {
          send: function send(reqhdr, completeCallback) {
  ...
  return {
    ...
    setIsDependentFunction: function setIsDependentFunction(func) {
      _isDependentFunction = func;
    },
    ready: function ready(readyHandler) {
    ...
    debug: function debug() {
      ...
      $.getScript(debuggerUrl).fail(function (jqxhr, settings, exception) {
    ...
    _internal: {
      ...
      initEventSource: function initEventSource(url) {
